...
public interface IMapper<T> extends BaseMapper<T> {
	...
	default int updateColumnsByBId(@Param(Constants.ENTITY) T entity, SFunction<T, ?>... columns) {
		...
		for (SFunction<T, ?> columnFunc : columns) {
			...
			entityItemList.add(new EntityItem(column, BeanUtils.getProperty(entity, fieldName)));
		}
	...
	default int updateBatchByBIds(List<T> entityList) {
		...
		for (T e : entityList) {
			if (entity == null) {
				entity = e;
				bIdFieldName = BIdUtils.getBIdFieldName(e.getClass());
			}
			if (StringUtils.isBlank(bIdFieldName)) {
				throw new MybatisPlusException("not found @TableBId in entity");
			}
			// 获取 bizId 的值
			Serializable bId = (Serializable) BeanUtils.getProperty(e, bIdFieldName);
			if (bId == null) {

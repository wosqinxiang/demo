...
import com.ahdms.framework.stream.client.StreamClientFactoryBean;
...
public class MappingJackson2MessageConverter extends AbstractMessageConverter {
    ...
    protected Object convertFromInternal(Message<?> message, Class<?> targetClass, @Nullable Object conversionHint) {
        Class<?> nestedClass = Optional.ofNullable(message.getHeaders())
                .map(headers -> headers.get(StreamClientFactoryBean.HEADER_NESTED_CLASS))
                .map(Class.class::cast).orElse(null);
        JavaType javaType = getJavaType(nestedClass, targetClass, conversionHint);
    ...
    private JavaType getJavaType(Class<?> nestedClass, Class<?> targetClass, @Nullable Object conversionHint) {
        if (nestedClass != null) {
            return this.objectMapper.getTypeFactory().constructParametricType(targetClass, nestedClass);
        }

package com.ahdms.framework.lock.locker;

import com.ahdms.framework.core.commom.util.StringUtils;
import com.ahdms.framework.lock.UnableToAcquireLockException;
import com.ahdms.framework.lock.utils.LockerUtils;
...
public class RedissonDistributedLockerClient implements DistributedLockerClient, InitializingBean {
    ...
    public String lock(String lockName, int leaseTime, int waitTime) throws UnableToAcquireLockException {
        lockName = LockerUtils.getLockName(lockName);
        ...
        try {
            ...
            throw new UnableToAcquireLockException(
                StringUtils.format("Failed to acquire distributed lock with name {}, lease time {}, wait time {} .", lockName, leaseTime, waitTime));
    ...
    public void afterPropertiesSet() throws Exception {
        Assert.notNull(this.defaultLeaseTime, "Default expire time must not be null, please configure with key \"dms.lock.defaultLeaseTime\"");
        Assert.notNull(this.defaultWaitTime, "Default expire time must not be null, please configure with key \"dms.lock.defaultWaitTime\"");

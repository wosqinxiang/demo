...
//        mapper.configure(SerializationFeature.WRITE_NULL_MAP_VALUES, false); // 不输出value=null的属性
		...
		mapper.setDateFormat(new SimpleDateFormat(DateUtils.PATTERN_MVC));
		mapper.setTimeZone(DateUtils.tzUTC);
		...
		timeModule.addDeserializer(LocalDate.class, new LocalDateDeserializer(DateUtils.DATE_FORMATTER));
		timeModule.addSerializer(LocalDate.class, new LocalDateSerializer(DateUtils.DATE_FORMATTER));

...
import com.ahdms.auth.model.BasicResultData;
...
import com.ahdms.auth.common.Base64Utils;
import com.ahdms.auth.common.UUIDGenerator;
...
import com.ahdms.identification.utils.AsnUtil;
...
//		byte[] inData = ASN1OctetString.getInstance(signedData.getContentInfo().getContent()).getOctets();
	...
	public String signVerifier(String httpResponse) throws ApiException {
		try {

			BasicResultData basicResultData = JsonUtils.json2Bean(httpResponse, BasicResultData.class);
			String sign = basicResultData.getSign();
			String bizPackage = basicResultData.getBizPackage();

			boolean encryptResult = signVerifier(sign, bizPackage.getBytes());
			if (encryptResult) { // 验签通过
				return bizPackage;
			}
		}catch (Exception e){
			logger.error(e.getMessage(),e);
		}
		throw new ApiException("数据验签失败！请重试！");
	}
